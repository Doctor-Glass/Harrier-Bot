...
# !time ET [destination timezone(s)] converts the current server time to any number of destination timezones
# !time [source timezone] [destination timezone(s)] can be used to convert the current time between a source timezone and an arbitrary number of destination timezones
...
def convert_time(args):
    ...
    if (len(args) == 1) :
        ...
        if args[0] in timezones_name:
            time_offset = timezones_name[args[0]]
        elif args[0] in timezones_country:
            time_offset = timezones_country[args[0]]
        else:
